...
# Test: Basic splinter functionality
test_basic_commit_to() {
    ...
    bash "$SCRIPT_DIR/git-splinter.sh" feature-branch -m "Test commit"
...
test_no_staged_changes() {
    ...
    if bash "$SCRIPT_DIR/git-splinter.sh" test-branch 2>&1 | grep -q "No staged changes found"; then
        return 0
    else
        return 1
    fi
...
test_custom_commit_message() {
    ...
    bash "$SCRIPT_DIR/git-splinter.sh" message-branch -m "$test_message"
...
main() {
    # Run all tests
    run_test "Basic splinter functionality" test_basic_commit_to
